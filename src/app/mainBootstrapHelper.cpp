...
QString appShortcutPath()
{
    ...
    if (const HRESULT hr = SHGetKnownFolderPath(FOLDERID_Programs, KF_FLAG_DEFAULT, nullptr, &rawProgramsPath);
        FAILED(hr) || rawProgramsPath == nullptr) {
...
bool writeRegistryDefaultValue(const std::wstring& subKey, const std::wstring& value)
{
    ...
    const LONG createResult = RegCreateKeyExW(
            HKEY_CURRENT_USER, subKey.c_str(), 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &key, nullptr);
...
bool MainBootstrapHelper::ensureToastShortcut(const wchar_t* appUserModelId)
{
    ...
    if (exePath.isEmpty() || shortcutPath.isEmpty()) {
        return false;
    }
    ...
    if (SUCCEEDED(hr)) {
        hr = shellLink->SetIconLocation(exeWide.c_str(), 0);
    }
    ...
    if (SUCCEEDED(hr)) {
        hr = InitPropVariantFromString(appUserModelId, &appIdProp);
    }
    if (SUCCEEDED(hr) && propertyStore != nullptr) {
        hr = propertyStore->SetValue(PKEY_AppUserModel_ID, appIdProp);
    }
    if (SUCCEEDED(hr) && propertyStore != nullptr) {
        hr = propertyStore->Commit();
    }
    if (const HRESULT clearHr = PropVariantClear(&appIdProp); FAILED(clearHr)) {
        LOG_WARNING() << "PropVariantClear failed. HRESULT=0x" << QString::number(static_cast<qulonglong>(clearHr), 16);
    }
...
void MainBootstrapHelper::ensureProtocolRegistration()
{
    ...
    const std::wstring command = QString(R"("%1" "%2")").arg(QDir::toNativeSeparators(exePath), "%1").toStdWString();

    bool ok = true;
    ok = writeRegistryDefaultValue(baseKey, QString("URL:%1 protocol").arg(AppSettings::APP_NAME).toStdWString()) && ok;
    ok = writeRegistryNamedValue(baseKey, L"URL Protocol", L"") && ok;
    ok = writeRegistryDefaultValue(baseKey + L"\\DefaultIcon",
                                   QDir::toNativeSeparators(exePath).toStdWString() + L",0") &&
         ok;
...
QStringList MainBootstrapHelper::startupArgsFromArgv(const int argc, char* const* argv)
{
    ...
    for (const std::span<char* const> rawArgs(argv, static_cast<size_t>(argc)); char* const arg : rawArgs.subspan(1)) {
        args.push_back(QString::fromLocal8Bit(arg));
    }
    return args;
}
...
QString MainBootstrapHelper::instanceLockPath()
{
    ...
    return QDir(lockDir).filePath(QString("%1.single_instance.lock").arg(QString(AppSettings::APP_NAME).toLower()));
}
...
bool MainBootstrapHelper::isToastActivationLaunch(const QStringList& startupArgs)
{
    ...
    return std::ranges::any_of(startupArgs, [&toastArg, &toastUri](const QString& arg) {
