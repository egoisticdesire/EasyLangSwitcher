...
bool TrayManager::shouldDeferPopupNotification() {
    ...
    const auto [
        ...
        shouldDefer
    ] = WindowsNotificationState::evaluatePopupDeferral();
    ...
    LOG_DEBUG() << "Notification state: "
                ...
                << "; toast-setting=" << toastSettingName
...
void TrayManager::showGlobalNotification(const GlobalNotification::Mode mode, const QString &version,
                                         const QString &url) {
...
void TrayManager::clearAvailableUpdate() {
    const bool hadAvailable = m_hasAvailableUpdate
                              || !m_availableUpdateVersion.isEmpty()
                              || !m_availableUpdateUrl.isEmpty();
...
QIcon TrayManager::buildTrayIcon() const {
    ...
    if (badgedIcon.isNull()) return base;
    return badgedIcon;
}
