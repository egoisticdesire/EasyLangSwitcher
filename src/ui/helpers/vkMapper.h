...
{
    static const QHash<int, QString> map = {{VK_LCONTROL, "Left Ctrl"},
...
inline const QHash<QString, int>& nameToVkMap()
{
    ...
    if (reverse.isEmpty()) {
        ...
        for (char c = 'A'; c <= 'Z'; c++) {
            reverse.insert(QString(c), static_cast<unsigned char>(c));
        }
...
inline int sequenceToVk(const QKeySequence& seq)
{
    ...
    if (const QKeyCombination comb = seq[0]; // NOLINT(cppcoreguidelines-pro-bounds-avoid-unchecked-container-access)
        (comb.key() >= Qt::Key_A && comb.key() <= Qt::Key_Z) || (comb.key() >= Qt::Key_0 && comb.key() <= Qt::Key_9)) {
        return comb.key();
    }
