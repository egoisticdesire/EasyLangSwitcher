...
namespace
{
enum class QuietHoursProfile : std::uint8_t
{
    Unknown,
    Unrestricted,
    PriorityOnly,
...
QString notificationStateName(const QUERY_USER_NOTIFICATION_STATE state)
{
    switch (state) {
        ...
        case QUNS_RUNNING_D3D_FULL_SCREEN:
            return "running_d3d_full_screen";
...
{
    switch (profile) {
        case QuietHoursProfile::Unrestricted:
            return "unrestricted";
        case QuietHoursProfile::PriorityOnly:
            return "priority_only";
        case QuietHoursProfile::AlarmsOnly:
            return "alarms_only";
        case QuietHoursProfile::Other:
            return "other";
...
bool readRegistryDwordValue(const HKEY rootKey, const wchar_t* subKey, const wchar_t* valueName, DWORD& value)
{
    DWORD data = 0;
    DWORD dataSize = sizeof(data);
    if (const LSTATUS status = RegGetValueW(rootKey, subKey, valueName, RRF_RT_REG_DWORD, nullptr, &data, &dataSize);
...
bool readRegistryBinaryValue(const HKEY rootKey,
                             const wchar_t* subKey,
                             const wchar_t* valueName,
                             std::vector<BYTE>& value)
{
    DWORD dataSize = 0;
    LSTATUS status = RegGetValueW(rootKey, subKey, valueName, RRF_RT_REG_BINARY, nullptr, nullptr, &dataSize);
    if (status != ERROR_SUCCESS || dataSize == 0) {
        return false;
    }

    std::vector<BYTE> data(dataSize);
    status = RegGetValueW(rootKey, subKey, valueName, RRF_RT_REG_BINARY, nullptr, data.data(), &dataSize);
...
bool isForegroundFullscreenWindow()
{
    ...
    return windowRect.left <= monitorInfo.rcMonitor.left && windowRect.top <= monitorInfo.rcMonitor.top &&
           windowRect.right >= monitorInfo.rcMonitor.right && windowRect.bottom >= monitorInfo.rcMonitor.bottom;
}
...
{
    static constexpr auto kDataValue = L"Data";
...
{
    static constexpr auto kCurrentPath =
            L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CloudStore\\Store\\DefaultAccount\\Current\\default$"
            L"windows.data.donotdisturb.quiethourssettings\\windows.data.donotdisturb.quiethourssettings";
    static constexpr auto kLegacyCachePath =
            L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\CloudStore\\Store\\Cache\\DefaultAccount\\$$windows.data."
            L"notifications.quiethourssettings\\Current";
...
{
    DWORD state = 0;
    if (!readRegistryDwordValue(HKEY_CURRENT_USER,
...
bool isFocusSessionActive()
{
    ...
    HRESULT hr = RoGetActivationFactory(HStringReference(RuntimeClass_Windows_UI_Shell_FocusSessionManager).Get(),
                                        IID_PPV_ARGS(&focusManagerStatics));
    if (FAILED(hr) || focusManagerStatics == nullptr) {
        return false;
    }

    boolean isSupported = 0U;
    hr = focusManagerStatics->get_IsSupported(&isSupported);
    if (FAILED(hr) || isSupported == 0U) {
        return false;
    }

    ComPtr<IFocusSessionManager> focusManager;
    hr = focusManagerStatics->GetDefault(&focusManager);
    if (FAILED(hr) || focusManager == nullptr) {
        return false;
    }
...
{
    if (!ensureRuntimeInitialized()) {
        return NotificationSetting_Enabled;
    }
    ...
    if (FAILED(hr) || toastManager == nullptr) {
        return NotificationSetting_Enabled;
    }
...
WindowsNotificationEvaluation WindowsNotificationState::evaluatePopupDeferral()
{
    ...
    result.deferBySystemState = (result.hasSystemNotificationState && state == QUNS_NOT_PRESENT) ||
    ...
    result.deferByQuietHoursProfile =
            quietHoursProfile != QuietHoursProfile::Unknown && quietHoursProfile != QuietHoursProfile::Unrestricted;

    result.shouldDefer = result.deferBySystemState || result.deferByFullscreenWindow ||
                         result.deferByQuietHoursService || result.deferByGlobalToastSetting ||
                         result.deferByFocusSession || result.deferByToastSetting || result.deferByQuietHoursProfile;
