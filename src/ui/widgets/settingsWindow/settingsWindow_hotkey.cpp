...
void SettingsWindow::initHotkeyLogic()
{
    ...
    connect(m_keyHelper,
            &KeySequenceHelper::hotkeySelected,
            this,
            [this](const int mainVk, int /*mods*/, const QString& name) {
                ...
                if (const auto it = std::find(presetMap.constBegin(), presetMap.constEnd(), AppSettings::hotkeyMainVk);
                    ...
                    LOG_DEBUG() << "Saved previous preset: vk=" << AppSettings::previousHotkeyMainVk << "; name='"
...
void SettingsWindow::buildPresetMap()
{
    ...
    for (const auto presetButtons = ui.key_select_frame->findChildren<QPushButton*>();
        ...
        int vk = 0;
        if (obj.contains("lctrl"))
            vk = VK_LCONTROL;
        else if (obj.contains("rctrl"))
            vk = VK_RCONTROL;
        else if (obj.contains("lalt"))
            vk = VK_LMENU;
        else if (obj.contains("ralt"))
            vk = VK_RMENU;
        else if (obj.contains("lshift"))
            vk = VK_LSHIFT;
        else if (obj.contains("rshift"))
            vk = VK_RSHIFT;
        else if (obj.contains("caps"))
            vk = VK_CAPITAL;

        if (vk != 0) {
            presetMap.insert(btn->objectName(), vk);
        }
    }
    ...
    if (AppSettings::previousHotkeyMainVk != 0) {
        if (AppSettings::hotkeyMainVk != AppSettings::previousHotkeyMainVk) {
            ...
            LOG_DEBUG() << QString("Previous preset restored: vk=%1; name='%2'")
                                   .arg(AppSettings::previousHotkeyMainVk)
                                   .arg(AppSettings::previousHotkeyName);
...
{
    if (ui.content_container->currentIndex() == 0) {
